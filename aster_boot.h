...
">r >r swap >r ; compile-only create cstack 200 cells allot cstack 200 + "
"constant cstacktop cstack value csp : >c csp dup cstacktop >= if .\" "
"compile stack overflow\" cr cstack to csp -1 error then ! csp cell+ to csp "
"; compile-only : c> csp cell - dup dup cstack < if .\" compile stack "
"underflow\" cr cstack to csp -1 error then to csp @ ; compile-only : case 0 "
">c ; immediate compile-only : of postpone over postpone = r> postpone if >r "
"postpone drop c> 1+ >c ; immediate compile-only : endof r> postpone else >r "
"; immediate compile-only : endcase postpone drop r> c> begin ?dup while 1- "
"postpone then repeat >r ; immediate compile-only : str= rot over <> if "
"2drop drop 0 exit then 0 ?do over i + c@ over i + c@ <> if 2drop unloop 0 "
"exit then loop 2drop -1 ; : key cin begin cin 10 = until ; : accept dup >r "
"0 ?do cin dup 10 = over -1 = or if 2drop i unloop r> drop exit then over i "
"+ c! loop drop r> ; : throw if .\" throw\" cr -1 error then ; : bin 3 + ; ' "
...
"168 constant pad-size create pad pad-size allot : begin here >c ; immediate "
"compile-only : again c> jmp, ; immediate compile-only : until c> jz, ; "
"immediate compile-only : repeat r> c> jmp, here r> cell - ! >r ; immediate "
"compile-only : esc[ 27 emit [char] [ emit ; : page esc[ .\" 2J\" esc[ .\" "
"H\" ; : at-xy esc[ .\" H\" ?dup if esc[ (.) type .\" B\" then ?dup if esc[ "
"(.) type .\" C\" then ; ";
